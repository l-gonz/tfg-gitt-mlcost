...
@click.option('-d', '--dataset', help='filepath to dataset, uses Iris dataset if none given')
@click.option('-l', '--labels', help='labels column, defaults to last column')
@click.option('-t', '--test', help='filepath to testset, if none will get split testset from dataset')
...
@click.option('-f', '--file', help='filepath to csv file that contains the data', required=True)
def show(file):
    graphs.main(file)
